...
import de.uma.dcsim.scheduling.schedulingStrategies.schedulingUtilities.JobSortContainer;
...
/**
 * This class implements a scheduling strategy that uses a heurist, which minimizes the SLA costs of a job, to determine
 * the order in which the jobs are scheduled.
 * 
 * @author nilsw
 *
 */
public class ShortestTimeToDeadlineFirst implements SchedulingStrategy {
	...
	public void scheduleNextInterval(List<BatchJob> runningJobs, List<BatchJob> submittedJobs, List<BatchJob> scheduledJobs, List<BatchJob> pausedJobs, int intervalLength, int currentTime, int[] nodeOccupationPlan, DC handledDC) {
		...
		List<JobSortContainer> sortedJobs = SchedulingStrategyUtilities.sortJobListByTheta(allJobs);
